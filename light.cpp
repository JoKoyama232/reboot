...
void InitLight(void)
{
	...
	for (int i = 0; i < LIGHT_MAX; i++)
	{
		...
		g_Light[i].Attenuation = 100000.0f;	// 減衰距離
	...
	// 並行光源の設定（世界を照らす光）
	g_Light[0].Direction = XMFLOAT3( 0.0f, 0.0f, 0.0f );		// 光の向き
	g_Light[0].Position = XMFLOAT3(0.0f, 500.0f, 0.0f);
	g_Light[0].Diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);	// 光の色
	g_Light[0].Ambient = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	g_Light[0].Type = LIGHT_TYPE_POINT;					// 並行光源
	...
	g_Light[1].Ambient = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	...
	g_Fog.FogColor = XMFLOAT4( 0.5f, 0.5f, 0.75f, 0.01f );		// フォグの色
	SetFog(&g_Fog);
