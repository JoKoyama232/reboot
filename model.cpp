...
// 初期化処理
//=============================================================================
void LoadModel( char *FileName, DX11_MODEL* modelGpu, MODEL* modelCpu)
{
	MODEL model;
	...
	// 頂点バッファ生成
	{
		...
		GetDevice()->CreateBuffer( &bd, &sd, &modelGpu->VertexBuffer );
	}
	...
	// インデックスバッファ生成
	{
		...
		GetDevice()->CreateBuffer( &bd, &sd, &modelGpu->IndexBuffer );
	}

	// サブセット設定
	{
		modelGpu->SubsetArray = new DX11_SUBSET[ model.SubsetNum ];
		modelGpu->SubsetNum = model.SubsetNum;

		for( unsigned short i = 0; i < model.SubsetNum; i++ )
		{
			modelGpu->SubsetArray[i].StartIndex = model.SubsetArray[i].StartIndex;
			modelGpu->SubsetArray[i].IndexNum = model.SubsetArray[i].IndexNum;

			modelGpu->SubsetArray[i].Material.Material = model.SubsetArray[i].Material.Material;

			D3DX11CreateShaderResourceViewFromFile( GetDevice(),
													model.SubsetArray[i].Material.TextureName,
													NULL,
													NULL,
													&modelGpu->SubsetArray[i].Material.Texture,
													NULL );
		}
	}
